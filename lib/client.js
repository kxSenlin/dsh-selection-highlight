window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const DEFAULT_HIGHLIGHT_OPACITY = .45;
		...
		function normalizeColor(value) {
			return typeof value === "string" && /^#[0-9a-f]{6}$/i.test(value) ? value : DEFAULT_HIGHLIGHT_COLOR;
		}
		function normalizeOpacity(value) {
			if (typeof value !== "number" || !Number.isFinite(value)) return DEFAULT_HIGHLIGHT_OPACITY;
			return Math.min(1, Math.max(.05, Math.round(value * 100) / 100));
		}
		...
		function ensureHighlightStyle(color, opacity) {
			const css = [
...
`;
		...
		function makeRow(title, description, control, vertical = false) {
		...
		function createSettingsPanel(controller) {
			...
			const colorRow = makeRow("高亮颜色", "设置高亮色与不透明度；拖动滑杆即时预览，修改立即生效。", colorControl);
			...
			const selectorRow = makeRow("对话区域 CSS 选择器", "dsh 改版后可手动指定消息容器。默认值对应当前会话列的消息流。", selectorControl, true);
			const refreshStatus = () => {
